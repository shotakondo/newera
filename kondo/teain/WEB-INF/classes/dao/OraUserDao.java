...
public class OraUserDao implements UserDao{
	...
	public void EditUser(UserBean ub){
		...
		try{
			...
			String sql = "UPDATE user_table SET user_firstname = ?, user_lastname = ?, user_email = ?, user_tel = ?, user_postcode = ?, user_address = ?, user_sex = ?, user_birthday = ? where user_id = '"+ ub.getUid() + "'";
			
			st = cn.prepareStatement(sql);

			st.setString(1,ub.getFirsname());
			st.setString(2,ub.getLastname());
			st.setString(3,ub.getEmail());
			st.setString(4,ub.getTel());
			st.setString(5,ub.getPostcode());
			st.setString(6,ub.getAddress());
			st.setString(7,ub.getSex());
			st.setString(8,ub.getBirthday());
